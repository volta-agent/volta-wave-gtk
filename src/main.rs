use gtk4::{glib, prelude::*, Application, ApplicationWindow, Box, Button, Label, ListBox, ListBoxRow, Orientation, Scale, DrawingArea, Entry, CssProvider, MenuButton, Image, Overlay, EventControllerKey};
use glib::Propagation;
use gtk4::gdk::{Display, MemoryTexture, MemoryFormat};
use gtk4::cairo;
...
fn build_ui(app: &Application) {
    ...
    library_tab.connect_clicked(glib::clone!(
        ...
        @strong queue_controls_clone => move |_| {
    ...
    save_playlist_btn.connect_clicked(glib::clone!(@strong tracks_for_save => move |_| {
        ...
        dialog.connect_response(glib::clone!(@strong entry => move |dialog, response| {
            if response == gtk4::ResponseType::Ok {
                ...
                if !name.is_empty() {
                    let tracks_ref: Vec<Track> = tracks.iter().map(|t| Track {
                        path: t.path.clone(),
                        title: t.title.clone(),
    ...
    load_playlist_btn.connect_clicked(glib::clone!(@strong track_list, @strong track_rows, @strong track_count_label => move |_| {
    ...
    key_controller.connect_key_pressed(glib::clone!(@strong pipeline_clone, @strong state_clone, @strong play_btn_clone => move |_, key, _, _| {
        match key {
            gtk4::gdk::Key::space => {
                let is_playing = state_clone.lock().unwrap().is_playing;
                if is_playing {
            ...
            gtk4::gdk::Key::Right | gtk4::gdk::Key::d => {
                // Seek forward
                if let Some(p) = pipeline_clone.lock().unwrap().as_ref() {
                    if let Some(playbin) = p.by_name("playbin") {
                        if let Some(pos) = playbin.query_position::<gstreamer::ClockTime>() {
...
fn create_track_row(track: &Track) -> ListBoxRow {
    ...
    vbox.append(&title);
    vbox.append(&artist);
    ListBoxRow::builder().child(&vbox).css_classes(vec!["track-row".to_string()]).build()
}
